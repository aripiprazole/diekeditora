...
class RoleAuthorityLoader(val roleService: RoleService) :
    KotlinDataLoader<Role, Set<String>> by dataLoader("RoleAuthorityLoader").execute({ role ->
        roleService.findRoleAuthorities(role)
    })

@Component
class UserAuthorityLoader(val userService: UserService) :
    KotlinDataLoader<User, Set<String>> by dataLoader("UserAuthorityLoader").execute({ user ->
        userService.findUserAuthorities(user)
    })

@Component
class AllUserAuthorityLoader(val authorityService: AuthorityService) :
    KotlinDataLoader<User, Set<String>> by dataLoader("AllUserAuthorityLoader").execute({ user ->
        authorityService.findAllAuthoritiesByUser(user)
    })
